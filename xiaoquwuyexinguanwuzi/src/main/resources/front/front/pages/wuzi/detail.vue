			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.wuziName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.wuziContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
				...
				<view class="bottom-content cu-bar bg-white tabbar border shop">
					<view style="width: 100%;margin: 0 auto;display: flex;align-items: center;justify-content: space-around;">
						...
						<view class="" style="display: flex; width: 70%;color: #FFFFFF;">
							<view @tap="juanzheng()" :style="[{backgroundImage:'linear-gradient(40deg,'+ btnColor[0] +','+ btnColor[1] +')'}]" style="width:47%;height: 80rpx;line-height: 80rpx;text-align: center;border-bottom-left-radius: 40rpx;border-top-left-radius: 40rpx;">
								捐赠物资
							</view>
							<view @tap="shenqing()" :style="[{backgroundImage:'linear-gradient(130deg,'+ btnColor[1] +','+ btnColor[0] +')'}]" style="width: 47%;height: 80rpx;line-height: 80rpx;text-align: center;border-bottom-right-radius: 40rpx;border-top-right-radius: 40rpx;">
								申请物资
							</view>
						</view>
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.wuziId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getwuziCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
						...
						wuziLiuyanPingfenNumber: this.scoreData,
                        wuziLiuyanText: this.commentData,
                    }
                    await this.$api.save(`wuziLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('wuzi', this.wuziId);
                    this.detail = res.data
					this.detail.wuziContent = this.detail.wuziContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.wuziPhoto ? this.detail.wuziPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getwuziCollection() {
					let params = {
						page: 1,
						limit: 1,
						wuziId: this.wuziId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`wuziCollection`, params);
					if (res.data.list.length == 1) {
						let wuziCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('wuziCollection', JSON.stringify([wuziCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getwuziCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('wuziCollection', {
									yonghuId: _this.user.id,
									wuziId: _this.detail.id,
									wuziCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('wuziLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						wuziId: this.wuziId
					});
