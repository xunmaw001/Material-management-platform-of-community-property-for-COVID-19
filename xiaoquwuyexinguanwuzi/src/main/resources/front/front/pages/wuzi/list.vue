	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 24rpx","boxShadow":"0px 6rpx 12rpx var(--publicSubColor)","margin":"6rpx 0rpx","borderColor":"#ccc","backgroundColor":"rgba(255, 255, 255, 1)","color":"#333","textAlign":"left","borderRadius":"40rpx 0","borderWidth":"0","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.wuziName" type="text" placeholder="物资名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('wuzi','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('wuzi','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='wuziTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in wuziTypesList" :key="index" :class="wuziTypes===item.codeIndex?'tabActive':''"
                                  @tap="wuziTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
...
<view class="list-box6"
    ...
    <block v-for="(product,index) in list" :key="index">
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item"
              :style='{"padding":"20rpx 30rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 2% 20rpx 0","borderColor":"var(--publicMainColor)","backgroundColor":"rgba(255,255,255,1)","borderRadius":"1800rpx 1800rpx 0 0","borderWidth":"0 10rpx 20rpx","width":"48%","borderStyle":"solid","height":"auto"}'>
            <image
                    :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 0 8rpx 0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","borderRadius":"50% ","borderWidth":"0","width":"100%","borderStyle":"solid","height":"240rpx"}'
                    mode="aspectFill" :src="product.wuziPhoto?baseUrl+product.wuziPhoto.split(',')[0]:''">
            </image>
            <view
                    :style='{"padding":"8rpx 10rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"4rpx 0","borderColor":"red","backgroundColor":"var(--publicSubColor)","color":"rgba(0, 0, 0, 1)","textAlign":"center","borderRadius":"20rpx","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'
                    class="title hide1">{{product.wuziName}}
            </view>
            <view v-if="product.wuziNewMoney"
                  :style='{"padding":"8rpx 10rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"4rpx 0","borderColor":"red","backgroundColor":"var(--publicSubColor)","color":"red","textAlign":"center","borderRadius":"20rpx","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"34rpx","borderStyle":"solid"}'
                  class="title hide1">￥{{product.wuziNewMoney}}
            </view>
            <view v-else-if="product.wuziValue"
                  :style='{"padding":"8rpx 10rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"4rpx 0","borderColor":"red","backgroundColor":"var(--publicSubColor)","color":"rgba(0, 0, 0, 1)","textAlign":"center","borderRadius":"20rpx","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'
                  class="title hide1">{{product.wuziValue}}
            </view>
            <view style="display: flex;justify-content: space-between;">
                <text v-if="isAuth('wuzi','修改')" class="cuIcon-edit"
                      @click.stop="onUpdateTap(product.id)">修改
                </text>
                <text v-if="isAuth('wuzi','删除')" class="cuIcon-delete"
                      @click.stop="onDeleteTap(product.id)">删除
                </text>
            </view>
        </view>
        <view @tap="onDetailTap(product)" v-if="index%6==1" class="list-item"
              :style='{"padding":"20rpx 30rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 0 20rpx 2%","borderColor":"var(--publicMainColor)","backgroundColor":"rgba(255,255,255,1)","borderRadius":"1800rpx 1800rpx 0 0 ","borderWidth":"0 10rpx 20rpx","width":"48%","borderStyle":"solid","height":"auto"}'>
            <image
                    :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 0 8rpx 0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","borderRadius":"50%","borderWidth":"0","width":"100%","borderStyle":"solid","height":"240rpx"}'
                    mode="aspectFill" :src="product.wuziPhoto?baseUrl+product.wuziPhoto.split(',')[0]:''">
            </image>
        ...
        <view @tap="onDetailTap(product)" v-if="index%6==4" class="list-item"
              :style='{"padding":"30rpx 20rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 0 20rpx","borderColor":"var(--publicMainColor)","backgroundColor":"rgba(255,255,255,1)","borderRadius":"20rpx 20rpx 0 0","borderWidth":"0 10rpx 20rpx","width":"100%","borderStyle":"solid","height":"auto"}'
              style="display: flex;">
            <image
                    :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","borderRadius":"20rpx","borderWidth":"0","width":"260rpx","borderStyle":"solid","height":"120rpx"}'
                    mode="aspectFill" :src="product.wuziPhoto?baseUrl+product.wuziPhoto.split(',')[0]:''">
            </image>
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 物资类型 字段 字典表查询方法
            async wuziTypesSelect(){
                let wuziTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'wuzi_types',
                });
                wuziTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.wuziTypesList = wuziTypes.data.list
            },
            //当前表的 物资类型字段类别搜索
			wuziTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.wuziTypes != 0) {
                    params.wuziTypes = this.wuziTypes
                }
                let _this = this
                let res = await this.$api.list(`wuzi`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('wuzi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.wuziShenheTime=shijian;
                            await _this.$api.update(`wuzi`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.wuziName) {
                    searchForm['wuziName'] = this.searchForm.wuziName
                }

                let res = await this.$api.list(`wuzi`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.wuziContent!=null){
                        item.wuziContent = item.wuziContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
