	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.wuziChuruInoutListId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('wuziChuruInoutList', this.wuziChuruInoutListId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.wuziChuruInoutListPhoto ? this.detail.wuziChuruInoutListPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('wuziChuruInoutListYuyue', {
									yonghuId: _this.user.id,
									wuziChuruInoutListId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/wuziChuruInoutListYuyue/list');
							}
						}
					});
				},
