	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.wuziChuruInoutId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('wuziChuruInout', this.wuziChuruInoutId);
                    this.detail = res.data
					this.detail.wuziChuruInoutContent = this.detail.wuziChuruInoutContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.wuziChuruInoutPhoto ? this.detail.wuziChuruInoutPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('wuziChuruInoutYuyue', {
									yonghuId: _this.user.id,
									wuziChuruInoutId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/wuziChuruInoutYuyue/list');
							}
						}
					});
				},
