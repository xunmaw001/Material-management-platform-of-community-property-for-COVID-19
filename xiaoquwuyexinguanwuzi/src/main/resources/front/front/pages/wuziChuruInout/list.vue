	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 24rpx","boxShadow":"0px 6rpx 12rpx var(--publicSubColor)","margin":"6rpx 0rpx","borderColor":"#ccc","backgroundColor":"rgba(255, 255, 255, 1)","color":"#333","textAlign":"left","borderRadius":"40rpx 0","borderWidth":"0","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.wuziChuruInoutName" type="text" placeholder="出入库名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('wuziChuruInout','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('wuziChuruInout','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='wuziChuruInoutTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in wuziChuruInoutTypesList" :key="index" :class="wuziChuruInoutTypes===item.codeIndex?'tabActive':''"
                                  @tap="wuziChuruInoutTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.wuziChuruInoutName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('wuziChuruInout','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <!-- 当前表 -->
                 <view v-show="item.wuziChuruInoutUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">出入库流水号</text>
                     <text style="width: 70%">
                         <text v-if="item.wuziChuruInoutUuidNumber != null">{{item.wuziChuruInoutUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.wuziChuruInoutValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">出入库类型</text>
                     <text style="width: 70%">{{item.wuziChuruInoutValue}}</text>
                 </view>
                <view v-show="item.wuziChuruInoutContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">备注</text>
                    <view style="width: 70%" v-html="item.wuziChuruInoutContent"></view>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('wuziChuruInout','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('wuziChuruInout','审核') && item.wuziChuruInoutYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
                ...
                this.wuziChuruInoutTypesSelect()
		...
		methods: {
            ...
            //当前表的 出入库类型 字段 字典表查询方法
            async wuziChuruInoutTypesSelect(){
                let wuziChuruInoutTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'wuzi_churu_inout_types',
                });
                wuziChuruInoutTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.wuziChuruInoutTypesList = wuziChuruInoutTypes.data.list
            },
            //当前表的 出入库类型字段类别搜索
			wuziChuruInoutTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.wuziChuruInoutTypes != 0) {
                    params.wuziChuruInoutTypes = this.wuziChuruInoutTypes
                }
                let _this = this
                let res = await this.$api.page(`wuziChuruInout`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('wuziChuruInout', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.wuziChuruInoutShenheTime=shijian;
                            await _this.$api.update(`wuziChuruInout`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.wuziChuruInoutName) {
                    searchForm['wuziChuruInoutName'] = this.searchForm.wuziChuruInoutName
                }

                let res = await this.$api.page(`wuziChuruInout`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.wuziChuruInoutContent!=null){
                        item.wuziChuruInoutContent = item.wuziChuruInoutContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
