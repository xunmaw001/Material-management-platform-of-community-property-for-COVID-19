	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('wuziYuyue', this.wuziYuyueId);
                    this.detail = res.data
					this.detail.wuziYuyueContent = this.detail.wuziYuyueContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.wuziYuyuePhoto ? this.detail.wuziYuyuePhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('wuziYuyueYuyue', {
									yonghuId: _this.user.id,
									wuziYuyueId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/wuziYuyueYuyue/list');
							}
						}
					});
				},
