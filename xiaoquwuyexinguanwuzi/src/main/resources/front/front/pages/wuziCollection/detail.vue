	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.wuziCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('wuziCollection', this.wuziCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.wuziCollectionPhoto ? this.detail.wuziCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('wuziCollectionYuyue', {
									yonghuId: _this.user.id,
									wuziCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/wuziCollectionYuyue/list');
							}
						}
					});
				},
