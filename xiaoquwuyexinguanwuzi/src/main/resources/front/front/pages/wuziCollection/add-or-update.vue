    ...
    <view class="content">
        <form class="app-update-pv">
                ...
                <view :style='{"boxShadow":"0","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(228, 232, 230, 1)","margin":"0 0 0px 0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"100rpx"}'
                    ...
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"var(--publicMainColor)","textAlign":"left"}'
                          class="title">类型</view>
                    <picker @change="wuziCollectionTypesChange" :value="wuziCollectionTypesIndex" :range="wuziCollectionTypesOptions" range-key="indexName">
                        <view
                                ...
                                class="uni-input">{{ruleForm.wuziCollectionTypes?ruleForm.wuziCollectionValue:"请选择类型"}}</view>
                    </picker>
                </view>
...
<script>
    ...
    export default {
		...
		/*下拉框*/
			let wuziCollectionTypesParams = {
                page: 1,
                limit: 100,
                dicCode: 'wuzi_collection_types',
            }
			let wuziCollectionTypes = await this.$api.page(`dictionary`, wuziCollectionTypesParams);
			this.wuziCollectionTypesOptions = wuziCollectionTypes.data.list
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`wuziCollection`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.wuziCollectionId){
                this.ruleForm.wuziCollectionId = options.wuziCollectionId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
                if (this.ruleForm.id) {
                    await this.$api.update(`wuziCollection`, this.ruleForm);
                } else {
                    await this.$api.save(`wuziCollection`, this.ruleForm);
                }
