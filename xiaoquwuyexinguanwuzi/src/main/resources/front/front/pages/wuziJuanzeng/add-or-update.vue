        ...
        <form class="app-update-pv">
                <!--<view :style='{"boxShadow":"0","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(228, 232, 230, 1)","margin":"0 0 0px 0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"100rpx"}'
                    ...
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"var(--publicMainColor)","textAlign":"left"}'
                          class="title">物资</view>
                    <picker @change="wuziChange" :value="wuziIndex" :range="wuziOptions" range-key="wuziName">
                        <view
                                ...
                                class="uni-input">{{ruleForm.wuziId?ruleForm.wuziName:"请选择物资"}}</view>
                    </picker>
                </view>-->
                <!--<view :style='{"boxShadow":"0","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(228, 232, 230, 1)","margin":"0 0 0px 0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"100rpx"}'
                      class="cu-form-group">
                ...
                <view
                    ...
                    <input   disabled
                             ...
                             :disabled="ro.wuziJuanzengNumber" type="number" v-model="ruleForm.wuziJuanzengNumber" placeholder="捐赠数量"></input>
                </view>
            <view
                ...
                <textarea :style='{"padding":"0 24rpx","boxShadow":"0px 6rpx 12rpx var(--publicSubColor)","margin":"6rpx 0rpx","borderColor":"#ccc","backgroundColor":"rgba(255, 255, 255, 1)","color":"#333","textAlign":"left","borderRadius":"40rpx 0","borderWidth":"0","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx","height":"200rpx"}'
                          :disabled="ro.wuziJuanzengContent" v-model="ruleForm.wuziJuanzengContent" placeholder="备注"/>
            </view>
...
<script>
    ...
    export default {
		...
		/*下拉框*/
			let wuziJuanzengYesnoTypesParams = {
                page: 1,
                limit: 100,
                dicCode: 'wuzi_juanzeng_yesno_types',
            }
			let wuziJuanzengYesnoTypes = await this.$api.page(`dictionary`, wuziJuanzengYesnoTypesParams);
			this.wuziJuanzengYesnoTypesOptions = wuziJuanzengYesnoTypes.data.list
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`wuziJuanzeng`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.wuziJuanzengId){
                this.ruleForm.wuziJuanzengId = options.wuziJuanzengId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            // 下拉变化
            wuziJuanzengYesnoTypesChange(e) {
            ...
            async onSubmitTap() {
				if ((!this.ruleForm.wuziJuanzengNumber) && (!this.$validate.isIntNumer(this.ruleForm.wuziJuanzengNumber)) && this.ruleForm.wuziJuanzengNumber >0) {
                    this.$utils.msg(`捐赠数量不能为空，不能小于0 格式为数字`);
                    return
                }
				if(uni.getStorageSync('wuziId')){
					this.ruleForm.wuziId = uni.getStorageSync('wuziId')
					uni.removeStorageSync('wuziId')
				}
                if (this.ruleForm.id) {
                    await this.$api.update(`wuziJuanzeng`, this.ruleForm);
                } else {
                    await this.$api.save(`wuziJuanzeng`, this.ruleForm);
                }
