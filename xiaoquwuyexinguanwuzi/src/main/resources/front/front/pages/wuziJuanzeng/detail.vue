	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.wuziJuanzengId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('wuziJuanzeng', this.wuziJuanzengId);
                    this.detail = res.data
					this.detail.wuziJuanzengContent = this.detail.wuziJuanzengContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.wuziJuanzengPhoto ? this.detail.wuziJuanzengPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('wuziJuanzengYuyue', {
									yonghuId: _this.user.id,
									wuziJuanzengId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/wuziJuanzengYuyue/list');
							}
						}
					});
				},
