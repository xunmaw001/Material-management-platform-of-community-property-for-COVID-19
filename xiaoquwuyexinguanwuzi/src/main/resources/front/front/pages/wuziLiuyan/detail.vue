	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('wuziLiuyan', this.wuziLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.wuziLiuyanPhoto ? this.detail.wuziLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('wuziLiuyanYuyue', {
									yonghuId: _this.user.id,
									wuziLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/wuziLiuyanYuyue/list');
							}
						}
					});
				},
